...
class BasicInformation
{
    ...
    public static function canSubmitMEPTPApplication(){
       ...
       ->join('batches', 'batches.id', 'm_e_p_t_p_applications.batch_id')->exists() && MEPTPApplication::where(['vendor_id' => Auth::user()->id])
       ->join('batches', 'batches.id', 'm_e_p_t_p_applications.batch_id')->where('m_e_p_t_p_applications.payment', false)->exists()){
            return $response = [
                'success' => false,
        ...
        if(MEPTPApplication::where(['vendor_id' => Auth::user()->id])
            ->join('batches', 'batches.id', 'm_e_p_t_p_applications.batch_id')->exists() && MEPTPApplication::where(['vendor_id' => Auth::user()->id])
            ->join('batches', 'batches.id', 'm_e_p_t_p_applications.batch_id')->where('batches.status', '=', true)->exists()){
            return $response = [
                'success' => false,
        ...
        if(MEPTPApplication::where(['vendor_id' => Auth::user()->id])
            ->join('batches', 'batches.id', 'm_e_p_t_p_applications.batch_id')->exists() && MEPTPApplication::where(['vendor_id' => Auth::user()->id])
            ->join('batches', 'batches.id', 'm_e_p_t_p_applications.batch_id')->where('m_e_p_t_p_applications.status', '!=', 'approved_card_generated')->exists()){
            return $response = [
                'success' => false,
                'message' => 'Currently not able to able submit new application. Your previous application still inprogress.',
            ];
        ...
        if(MEPTPApplication::where(['vendor_id' => Auth::user()->id])
            ->join('batches', 'batches.id', 'm_e_p_t_p_applications.batch_id')->exists() && MEPTPApplication::where(['vendor_id' => Auth::user()->id])
            ->join('batches', 'batches.id', 'm_e_p_t_p_applications.batch_id')->where('m_e_p_t_p_applications.status', '!=', 'approved_card_generated')->exists() && $batches->exists()){
            return $response = [
                'success' => false,
                'message' => 'Currently not able to able submit new application. Your previous application still inprogress.',
            ];
